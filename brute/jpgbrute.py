...
def find_potential(block_size, device):
	cmd = "sigfind -b " + str(block_size) + " ffd8ff " + device
	...
	with open("temp.temp") as sig_file:
		...
		for line in sig_file:
			if cnt_line != 0:
				...
				#obtained sector that starts with FFD8FF
				
				#now run dd to find sector that ends with FFD9
				counter = 1
				found = 0
				
				while found == 0:
					cmd_dd = "dd if=" + device + " bs=" + str(block_size) + " skip=" + str(starter_token) + " count=" + str(counter) + " | xxd | grep -i -w ffd9"
				...
				if found == 0:
					...
					newfile = "s"+str(starter_token)+"c"+str(trailer_token)+".jpg"
